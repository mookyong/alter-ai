# wbs_공휴일반영_데이터마이그레이션_WBS명변경

- 원본 엑셀: `wbs_공휴일반영_데이터마이그레이션_WBS명변경.xlsx`
- `실제 시작일`, `실제 종료일` 및 공휴일 날짜는 `yyyy-mm-dd` 형식으로 출력했습니다.
...
| 차수 | 기존 WBS Task | 변경 WBS Task | 변경 기준 |
| --- | --- | --- | --- |
| 1 | 개발환경 구성 | 개발환경 초기 구성 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 1 | 요구사항 분석 | 요구사항/현황 초기 수집 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 2 | DataStage 구조 추출 | DataStage 메타데이터 추출 설계 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 2 | 개발환경 구성 | 개발환경 표준 확정 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 2 | 요구사항 분석 | 요구사항 인터뷰 및 범위 초안 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 3 | DataStage SQL 및 시퀀스 추출 | DataStage SQL/시퀀스 추출 테스트 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 3 | DataStage 구조 분석 | DB2 소스/타겟 매핑 초안 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 3 | DataStage 구조 추출 | DataStage 분석 스크립트 1차 개발 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 4 | DataStage SQL 및 시퀀스 추출 | DataStage 전체 SQL/시퀀스 추출 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 4 | DataStage 구조 분석 | DataStage 구조/변환 유형 분석 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 5 | DataStage 구조 분석 결과 리뷰 | DataStage 분석 결과 리뷰 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 5 | Snowflake SQL 전환 유형 POC | Snowflake 전환 POC 샘플링 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 5 | 요구사항 확정 | 전환 범위 및 오픈 대상 확정 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 6 | AI Agent 활용 개발 방안 설계 | AI Agent 활용 범위 검증 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 6 | Airflow DAG 개발 표준 설계 | Airflow DAG 개발 표준 설계 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 6 | DBT 모델 개발 표준 설계 | dbt 모델 개발 표준 설계 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 6 | Snowflake SQL 전환 유형 POC | Snowflake DDL/DML 전환 POC | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 7 | AI Agent 활용 개발 방안 설계 | AI Agent 전환 절차 설계 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 7 | Airflow DAG 구현 | Airflow DAG 템플릿 구현 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 7 | DBT 모델 구현 | dbt 전환 템플릿 구현 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 8 | Airflow DAG 구현 | 1차 DAG 기본 구조 구현 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 8 | DBT 모델 구현 | 1차 공통/단순 SQL 모델 구현 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 8 | 테스트 | 1차 단위 테스트 준비 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 9 | Airflow DAG 구현 | 1차 시퀀스 기반 DAG 구현 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 9 | DBT 모델 구현 | 1차 복합 변환 모델 구현 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 9 | 테스트 | 1차 데이터 검증 수행 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 10 | DBT 모델 구현 | 1차 복잡 JOB 전환 및 2차 개발 착수 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 10 | DataStage 구조 분석 | 2차 대상 분석 재확인 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 11 | Airflow DAG 구현 | 1차 DAG 전체 실행 검증 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 11 | DBT 모델 구현 | 1차 모델 개발 완료 및 고난도 처리 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 11 | 테스트 | 1차 단위 테스트 종료 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 12 | DBT 모델 구현 | 2차 모델 병행 개발 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 12 | 테스트 | 1차 통합 테스트 및 리허설 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 13 | 1차 오픈 준비 | 1차 오픈 운영 준비 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 13 | DBT 모델 구현 | 2차 모델 지속 개발 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 13 | 테스트 | 1차 코드 프리즈/컷오버 리허설 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 14 | 1차 오픈 | 1차 오픈 수행 및 긴급 조치 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 14 | 테스트 | 1차 사후 검증/2차 통합 테스트 착수 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 15 | 1차 안정화 | 1차 운영 안정화 및 결함 처리 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 15 | 2차 오픈 준비 | 2차 오픈 리허설/체크리스트 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 15 | DBT 모델 구현 | 2차 모델 개발 완료 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 15 | 테스트 | 2차 통합 테스트 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 16 | 2차 오픈 | 2차 오픈 수행 및 장애 대응 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 16 | 테스트 | 2차 사후 검증 및 영향도 점검 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 17 | 전체 안정화 | 통합 배치 안정화/성능 튜닝 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 17 | 테스트 | 데이터 검증 자동화 보완 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 18 | Airflow DAG 보완 | 수작업 전환 대상 재검토 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 18 | DBT 모델 보완 | 잔여 모델/예외 로직 보완 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 18 | 운영 전환 | 운영 이관 문서 작성 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 18 | 테스트 | 도메인별 검증 결과 취합 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 19 | 운영 전환 | 운영 가이드/교육 자료 작성 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 19 | 최종 검증 | 최종 검증 리포트 초안 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 20 | 운영 이관 | 운영 이관 리허설/산출물 정리 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 20 | 최종 검증 | 최종 데이터 정합성 검증 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 20 | 프로젝트 종료 | 프로젝트 종료 준비 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 21 | 안정화 종료 | 하이퍼케어 종료 기준 확인 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 21 | 운영 이관 | 운영팀 최종 인수인계 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
| 21 | 프로젝트 종료 | 프로젝트 최종 보고/승인 | 기간/단계가 다르면 단계 목적을 반영해 별도 명칭 부여 |
