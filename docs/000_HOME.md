# Data Migration Project Home

## 바로가기

- [[001_DOCUMENT_INDEX]]
- [[002_PROJECT_DASHBOARD]]
- [[00_admin/project_overview]]
- [[00_admin/project_rules]]
- [[01_hands_on/part-1_overview]]
- [[01_hands_on/part-2_wsl_setup]]
- [[01_hands_on/part-3_vscode_setup]]
- [[01_hands_on/part-4_obsidian_setup]]
- [[01_hands_on/part-5_document_workflow]]
- [[01_hands_on/part-6_opencode_agent]]
- [[01_hands_on/part-7_subagent_practice]]
- [[01_hands_on/part-8_graphify_integration]]
- [[01_hands_on/part-9_code_review_graph]]
- [[01_hands_on/part-10_operational_standards]]

## PL 일일 관리

- [[002_PROJECT_DASHBOARD]]
- [[07_management/action_items]]
- [[08_meetings/weekly]]
- [[09_reports/weekly_report]]

## 운영 원칙

- 문서는 `docs/` 아래에서만 관리한다.
- Obsidian은 `docs/` Vault를 기준으로 사용한다.
- VSCode는 프로젝트 루트 전체를 연다.
- OpenCode 결과는 초안이며 PL이 최종 판단한다.
- 모든 공식 변경은 Git commit으로 남긴다.
